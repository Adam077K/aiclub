...
|-------------|---------|---------|
| `Aiclub` | Display name in agent prompts and brand voice | `Acme` |
| `aiclub` | lower-case slug used in env vars / urls | `acme` |
| `AICLUB` | UPPER prefix for env vars (e.g. `ACME_API_KEY`) | `ACME` |
| `AICLUB-` | Linear / Jira ticket prefix | `ACME-` |
| `aiclub.com` | Production domain | `acme.com` |
| `https://aiclub.com` | Marketing site URL (often a separate platform) | `https://acme.com` |
| `https://github.com/Adam077K/aiclub` | GitHub repo URL | `https://github.com/org/repo` |
...
| `.claude/hooks/*.sh` and `*.js` | Hooks run on every action. Read each one before enabling. `gsa-context-monitor.js` references env var `AICLUB_NO_AUTOCOMPACT` — set it or remove the check. |

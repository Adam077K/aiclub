...
# "feat(api): rate-limit free scans (AICLUB--104)"
```
...
```json
{
  ...
  "commits": [
    "feat(api): rate-limit free scans to 5 per IP per hour (AICLUB--104)",
...
|---|---|
| Touching `apps/web/supabase/migrations/` or RLS | `supabase-rls-aiclub` |

...
git commit -m "feat(ai/scan): implement scan-analyzer agent with eval (AICLUB--112)"
```
...
```json
{
  ...
  "commits": [
    "feat(ai/scan): implement scan-analyzer agent — extracts mention sentiment from AI search results (AICLUB--112)",
...
|---|---|
| RAG retrieval / pgvector queries | `pgvector-rag-aiclub` |

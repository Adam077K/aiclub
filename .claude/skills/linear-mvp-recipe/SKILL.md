...
description: "Aiclub's locked Linear label vocabulary, sub-ticket creation pattern, and single-synthesis-comment style. Use whenever any CEO or C-suite agent interacts with Linear tickets, creates sub-tickets, or posts comments."
tags: [workflow, aiclub-specific, linear, orchestration]
...
```
Aiclub — Main (product work)        ← Adam and agents work here
Aiclub — War Room (orchestration)   ← Standing Routines post here, separate from product
Aiclub — Strategy (board meetings)  ← Board meeting outputs
```

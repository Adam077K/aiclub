...
```bash
pnpm -F @aiclub/web test apps/web/__tests__/<specific-file>.test.ts
```
...
git commit -m "test(api/scan): add unit tests for /api/scan/start auth + validation (AICLUB--104)"
```
...
```json
{
  ...
  "commits": [
    "test(api/scan): add 8 unit tests for /api/scan/start auth, validation, and happy path (AICLUB--104)"
  ],
  ...
  "decisions_made": [
    {
      ...
      "reason": "Playwright sandbox not configured in CI for this branch; QA-Lead approved unit-only scope for AICLUB--104"
    }
  ],
  "blockers": []
}

...
Note: Phase 3 will add the `aiclub-brand-quality-bar` skill. Until it ships, apply the billion-dollar quality bar manually: every spacing value, color token, and font choice must be intentional and match `docs/BRAND_GUIDELINES.md`.
...
```bash
pnpm -F @aiclub/web dev &   # start dev server if not running
```
...
git commit -m "design(scan): implement scan results screen with tier score cards (AICLUB--N)"
```
...
```json
{
  ...
  "commits": [
    "design(scan): implement scan results page with per-engine score cards (AICLUB--118)",
    "design(scan): add responsive mobile layout for ScanResultCard (AICLUB--118)"
  ],
  "screenshots": [

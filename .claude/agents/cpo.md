...
6. Linear ticket via `mcp__linear__get_issue` (if AICLUB--N is referenced in brief)
...
```
Reach:      How many Aiclub users or prospects are affected per quarter?
...
## Solution
[What Aiclub builds — what it does and explicitly does NOT do]
...
| Writing customer-visible copy in a spec | `aiclub-voice-canon` |
...
- **DO NOT assume Stripe.** Aiclub uses Paddle exclusively. Any spec referencing billing must use Paddle terminology: `subscription`, `checkout`, `price_id`, `14-day money-back guarantee`.

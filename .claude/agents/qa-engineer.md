...
```bash
pnpm -F @aiclub/web test --listTests   # or grep for existing test files touching the changed code
```
...
git commit -m "test(api/agents): add coverage for credit-guard execution path (AICLUB--N)"
```
...
```json
{
  ...
  "commits": [
    "test(api/agents): add 6 tests for credit-guard execution path — 401, 402, 422, happy (AICLUB--117)"
  ],

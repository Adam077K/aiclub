...
2. `CLAUDE.md` — Aiclub stack, Supabase table names, file path conventions
...
git commit -m "docs(api): add scan/start reference doc (AICLUB--212)"
```
...
```json
{
  ...
  "commits": [
    "docs(api): add scan/start reference doc with rate-limit error codes (AICLUB--212)"
  ],
